...
test('Button element must function independently of form submission and validate required attributes', async ({ page }) => {
	...
	await page.locator(statusDisplaySelector).waitFor({ state: 'attached' });
	await page.evaluate((selector) => { document.querySelector(selector)!.textContent = ''; }, statusDisplaySelector);
